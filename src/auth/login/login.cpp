...
void login_user(const std::string &username, const std::unique_ptr<const std::string> &master_password) {
    ...
    for (auto &[msg_username, msg]: messages) {
        ...
        try {
            const unsigned char *key = RatchetSessionManager::instance().get_key_for_device(msg_username, msg->header);
            if (key != nullptr) {
                auto decrypted = decrypt_message_given_key(msg->ciphertext, msg->length, key);
                std::cout << "Decrypted message: " << bin2hex(decrypted.data(),
                                                              std::min(32, static_cast<int>(decrypted.size()))) <<
                        std::endl;
                delete[] key;
            }
