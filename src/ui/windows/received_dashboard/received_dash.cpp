...
void Received::refreshFileList()
{
    ...
    for (const auto& [file_name, file_size, mime_type, uuid] : metadata) {
        std::string file_size_str = std::to_string(file_size);
        addFileItem(QString::fromStdString(file_name), QString::fromStdString(file_size_str), "sadfa", "asdfadf", uuid, mime_type);
    }
}
