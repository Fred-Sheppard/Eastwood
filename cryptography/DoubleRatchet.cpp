...
void DoubleRatchet::dh_ratchet(const unsigned char* remote_public_key, bool is_sending) {
    // Generate new DH keypair if we're sending
    if (is_sending) {
        crypto_kx_keypair(local_dh_public, local_dh_private);
    }
    
    // Update remote public key if provided
    ...
    // Update keys based on shared secret
...
unsigned char* DoubleRatchet::message_send() {
    ...
    // Derive message key and update send chain
    return derive_message_key(send_chain.chain_key);
}

unsigned char* DoubleRatchet::message_receive(const unsigned char* new_remote_public_key) {
    // Perform DH ratchet step with new remote public key
    dh_ratchet(new_remote_public_key, false);
    
    // Derive message key and update receive chain
    return derive_message_key(recv_chain.chain_key);
}
...
void DoubleRatchet::print_state() const {
    ...
    std::cout << "Send key: ";
    for (int i = 0; i < crypto_kdf_KEYBYTES; i++)
        std::cout << std::hex << std::setw(2) << std::setfill('0') << (int)send_chain.chain_key[i];
    std::cout << std::endl;
    
    std::cout << "Recv key: ";
    for (int i = 0; i < crypto_kdf_KEYBYTES; i++)
        std::cout << std::hex << std::setw(2) << std::setfill('0') << (int)recv_chain.chain_key[i];
    std::cout << std::endl;
}
